...
#SBATCH --ntasks=18 ## number of cpu's to allocate for a job
#SBATCH --ntasks-per-node=18 ## number of cpu's to allocate per each node
...
gatk --java-options "-Xmx200G -XX:ParallelGCThreads=18 -Djava.io.tmpdr=temp" SplitNCigarReads \
                        -R ${REF} \
                        -I ${TAG}.MarkDuplicates.dedupped.bam \
                        -O ${TAG}.SplitNCigarReads.split.bam
done
