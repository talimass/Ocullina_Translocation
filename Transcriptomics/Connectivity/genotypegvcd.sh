...
gatk --java-options "-Xmx128G -XX:ParallelGCThreads=16" GenotypeGVCFs \
  -R ${REF} \
  -V cohort.g.vcf.gz -O cohort_genotypes.vcf.gz \
  -stand-call-conf 30 \
  --annotation AS_MappingQualityRankSumTest \
  --annotation AS_ReadPosRankSumTest > GenotypeGVCFs.log 2>&1
